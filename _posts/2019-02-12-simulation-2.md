...
This property is for the neighborhoods of the white nodes. 
It is not as powerful as the previous ones, but sometimes it can be crucial.

Consider a white node $w$ and two black neighbors $v_1$ and $v_2$. 
...
But they are a bit related, by the following property:

*Claim:*
Take a white node $w$.
For each edge around $w$, all the black neighbors agree on at least one label. 
That is for each edge, there is a label that is in all set labels. 

This label is simply the one that the white node would give if it were running  
the $T$-round algorithm in the real graph (not a simulated one).
...
The first one is this danger I warned you about in the common label property 
section: an edge is labeled differently in different simulations.
